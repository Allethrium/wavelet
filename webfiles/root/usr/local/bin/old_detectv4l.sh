...
# Update 10/27/2023 - Added an /inputs folder in ETCD and moved detection to UltraGrid for better handling of multiple inputs from same vendors.
# WIP, and is a major change - has broken the old way of doing things.  Encoder script handles this but other than that nothing should need modding..
...
string=$(/usr/local/bin/UltraGrid.AppImage --tool uv -t v4l2:help | grep /dev/vid &&)
