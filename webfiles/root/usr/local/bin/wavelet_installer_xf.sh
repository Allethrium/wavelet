...
rpm_overlay_install(){
	...
	podman build -t localhost/coreos_overlay --build-arg DKMS_KERNEL_VERSION=${DKMS_KERNEL_VERSION} -v /usr/local/bin:/usr/local/bin -v /var:/var -f /home/wavelet/containerfiles/Containerfile.coreos.overlay
