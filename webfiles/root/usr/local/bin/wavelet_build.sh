...
event_server(){
	...
	if [[ "$SERVER_FIRSTRUN_FLAG" == 1 ]]; then
		...
		# The server hash must be present, otherwise we'd write a malformed /UI/HOSTS// key.
		# If the conf was generated by an older/incomplete bootstrap this can be empty, so
		# we re-read the authoritative host hash from etcd before publishing the UI key.
		if [[ -z "$SVR_HOSTHASH" ]]; then
			echo "	WARN: SVR_HOSTHASH empty, retrieving authoritative server hash from etcd.."
			KEYNAME="/HOSTS/$hostNameSys"; read_etcd_global; SVR_HOSTHASH="$printvalue"
		fi
		if [[ -z "$SVR_HOSTHASH" ]]; then
			echo "	ERR: Server host hash still empty, skipping UIEnable write to avoid a malformed /UI/HOSTS/ key!"
		else
