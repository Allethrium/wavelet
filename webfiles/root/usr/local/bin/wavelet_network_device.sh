...
event_magewell_ndi(){
	...
	# Perhaps we should autogen an admin password and store it in etcd here?
	...
	curl --cookie /var/tmp/sid.txt "http://${ipAddr}/mwapi?method=add-user&id=Wavelet&pass=${waveletUserPass}"
...
event_ptz_ndiHX(){
		...
		# Populate values to etcd for this specific device
		UGdeviceStreamCommand="rtsp://${ipAddr}/1:554:decompress"
		populate_to_etcd
