...
event_libx265sw_low() {
	# Low bitrate if we are seeing excessive frame drops
	KEYNAME=uv_encoder
	KEYVALUE="libavcodec:encoder=libx265:preset=ultrafast:crf=30:gop=6:bitrate=15"
	write_etcd_global
	echo -e "x265 SVT Software mode activated, Bitrate 15M, decoder task restart bit set. \n"
	wavelet-decoder-reset
}	
...
# working on tweaking these values to something as reliable as possible.
	KEYNAME=uv_encoder
	KEYVALUE="libavcodec:encoder=hevc_qsv:low_power=1:gop=30"
...
event_rav1esw() {
	KEYNAME=uv_encoder
	KEYVALUE="libavcodec:encoder=librav1e:qp=50:tile-columns=2:tile-rows=2:speed=8"
	write_etcd_global
