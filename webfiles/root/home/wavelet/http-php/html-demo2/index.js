...
function fetchData() {
	// If demo_mock_data.js has loaded static mock data onto window, use it directly.
	// This lets the demo UI render fully when opened straight from disk (file://),
	// where fetch() to a PHP endpoint cannot work at all.
	if (window.DEMO_MOCK_DATA) {
		return Promise.resolve(processFetchedData(window.DEMO_MOCK_DATA));
	}
	...
	}).then(data => processFetchedData(data)).catch(error => {
...
async function handlePageLoad() {
	...
	populateBorderDemoBoxes();
	// Skip live SSE updates when running on static mock data (e.g. opened
	// directly from disk, file://) — there is no real server to stream
	// from, and EventSource cannot connect over file:// anyway.
	if (!window.DEMO_MOCK_DATA) {
...
function populateBorderDemoBoxes() {
	// TEMP: clone the real rendered group/host content into the border-style
	// comparison boxes so the borders can be judged against actual UI
	// elements instead of placeholder text. Delete alongside the .border-demo-a
	// / .border-demo-b markup and CSS once a border style is chosen.
	const source = document.getElementById('HostControlDiv');
	if (!source) return;
	document.querySelectorAll('.groups_divider.border-demo-a, .groups_divider.border-demo-b').forEach(target => {
		const demoClass = target.classList.contains('border-demo-a') ? 'border-demo-a' : 'border-demo-b';
		target.innerHTML = '';
		Array.from(source.childNodes).forEach(node => {
			const clone = node.cloneNode(true);
			// The actual visible "sample elements" are the individual group
			// cards (.groups_divider_inner), so the demo border class has to
			// go on each of those, not just the outer wrapper.
			if (clone.nodeType === 1 && clone.classList.contains('groups_divider_inner')) {
				clone.classList.add(demoClass);
			}
