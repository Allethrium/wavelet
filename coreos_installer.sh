...
echo -e "\n	Note: Put the drive controller for target devices in AHCI mode from BIOS setup! RAID or other modes have been observed to interfere with the process.\n"
...
if ls ./$FILEPREFIX* > /dev/null 2>&1; then
	...
	echo "No ISO found, downloading CoreOS ISO from internet, please be patient while this task executes..\n"
...
echo "Customizing ISO files with Ignition\n"
	if [[ "${serverMode}" = "1" ]]; then
		echo -e "\nProvisioning server and decoder, .ISO files output to ${HOME}/Downloads..\n"
		# Generate Server ISO
